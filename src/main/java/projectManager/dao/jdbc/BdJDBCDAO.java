...
import projectManager.dao.BdDAO;
...
public class BdJDBCDAO extends JdbcDaoSupport implements BdDAO {
    ...
    private ParameterizedRowMapper<Bd> bdParameterizedRowMapper = new ParameterizedRowMapper<Bd>() {
        @Override
        public Bd mapRow(ResultSet rs, int rowNum) throws SQLException {
            Bd bd = new Bd();

            bd.setIdBd(rs.getInt("id_bd"));
            bd.setBd(rs.getBytes("bd"));
    ...
    public Bd findByID(Integer id) {
        try {
            ...
            return getJdbcTemplate().queryForObject(findBd, bdParameterizedRowMapper);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public List<Bd> getAll() {
        List<Bd> result = getJdbcTemplate().query(DB, bdParameterizedRowMapper);
